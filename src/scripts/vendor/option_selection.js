...
Shopify.OptionSelectors.prototype.buildSelectors = function() {
  ...
  for (var i = 0; i < this.product.optionNames().length; i++) {
    ...
    createStyledOptions(sel);
  }
  function createStyledOptions(options){
    console.log('running');
    if(options.name == "Col"){
      for(var i = 0; i < options.values.length; i++){
        $('.options .selector-'+options.name).append('<div class="swatch-container"><button type="button" data-option="option'+(options.index + 1) +'" data-value="'+options.values[i]+'" class="swatch-option '+options.values[i].toLowerCase().replace(/ |\//g,'-').replace("'", "").replace("...", "").replace(".", "") +'"></button></div>');
      }     
      //Variant Switch
      $('.swatch-container:first-child').addClass('active');
      $('.selector-Color .swatch-option').click(function(){
        var selectedOption = $(this).data('option');
        var selectedValue = $(this).data('value');
        var selectedThumb = $('.thumbnails a[title="'+selectedValue+'"]').attr('title');
        selectedValue = selectedValue.replace("'", "");
        selectedValue = selectedValue.replace(/ [0-9]+$/, '');
        //Change Hidden Select Value
        $('.single-option-selector[data-option="'+selectedOption+'"]').val(selectedValue);
        $('.single-option-selector[data-option="'+selectedOption+'"]').trigger('change');
        $('#option-color option').val();
        //Set as Active
        $('.swatch-container.active').removeClass('active');
        $(this).parent().addClass('active');
        console.log(selectedValue);
        $('.thumbnails a[data-value="'+selectedValue+'"]').get(0).click();
      });
    }else{
      for(var i = 0; i < options.values.length; i++){
        $('.options .selector-'+options.name).append('<button type="button" data-option="option'+(options.index + 1) +'" data-value="'+options.values[i]+'">'+options.values[i]+'</button>');
        $('.options .selector-'+options.name+' button').click(function(){
          var selectedOption = $(this).data('option');
          var selectedValue = $(this).data('value');
          //Change Hidden Select Value
          $('.single-option-selector[data-option="'+selectedOption+'"]').val(selectedValue);
          $('.single-option-selector[data-option="'+selectedOption+'"]').trigger('change');
          //Set as Active
          $('.options .selector-'+options.name+' button.active').removeClass('active');
